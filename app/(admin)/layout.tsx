...
export default async function AdminLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      {/* Sidebar */}
      <aside className="hidden md:flex w-64 flex-col fixed inset-y-0 left-0 z-20 bg-white border-r border-slate-200/50">
        {/* Logo */}
        <div className="h-14 flex items-center px-4 border-b border-slate-200/50">
          <Link href="/dashboard" className="flex items-center gap-2">
            ...
            <span className="font-semibold tracking-tight">Pulse Admin</span>
          </Link>
        </div>
        ...
        <div className="p-4 border-t border-slate-200/50">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{session?.nome}</p>
      ...
      {/* Mobile header */}
      <header className="md:hidden fixed top-0 inset-x-0 z-20 h-14 bg-white border-b border-slate-200/50 flex items-center justify-between px-4">
        <Link href="/dashboard" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center">
            <span className="text-sm font-bold text-primary-foreground">P</span>
          </div>
